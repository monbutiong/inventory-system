  ...
  <table class="table table-bordered table-striped table-hover table-sticky">
    <thead class="main-thead">
      <tr>
        ...
        <?php if($quotation->confirmed==1){?>
          <th title="Total: <?=@$arr_rri ? count($arr_rri) : 0?>">Received</th>
          <th title="Total: <?=@$arr_iii ? count($arr_iii) : 0?>">Issued</th>
        <?php }?>
        <th>L/C Rate</th>
        <th width="100">FOB</th>
        <th>Discount</th> 
    ...
    <tbody>
      ...
      <tr onclick="show_q_items(<?=$lrs->id?>)" class="loc_area_<?=$lrs->id?> all_loc all_loc_header" style="cursor: pointer;">
        <td colspan="<?=@$quotation->confirmed==1 ? '15' : '13'?>" style="background-color: #999; color: #fff;">  
      ...
      <tr id="q_items<?=$rs->id?>" class="loc_area_<?=$lrs->id?> loc_area_body_<?=$lrs->id?> all_loc supp_<?=$rs->supplier?> all_rows">
        ...
        <?php if($quotation->confirmed==1){?>
          <td class="td_curreny"><?=@$arr_rri[$rs->inventory_id] ?? 0?></td>
          <td class="td_curreny"><?=@$arr_iii[$rs->inventory_id] ?? 0?></td>
        <?php }?>
      ...
      <tr class="loc_area_<?=$lrs->id?> loc_area_body_<?=$lrs->id?> all_loc all_ttl">
        <td colspan="<?=@$quotation->confirmed==1 ? '8' : '6'?>"></td>
      ...
      <tr style="background-color: #e6ebf2;">
         <td colspan="<?=@$quotation->confirmed==1 ? '16' : '14'?>"><b>TOTAL</b></td>  
    ...
    <thead>
      <tr>
        <th colspan="<?=@$quotation->confirmed==1 ? '23' : '21'?>" ><br/><br/></td>
      </tr>
      <tr onclick="show_q_items('local')" class="loc_area_local all_loc all_loc_header" style="cursor: pointer;">
        <th colspan="<?=@$quotation->confirmed==1 ? '23' : '21'?>" style="background-color: #999; color: #fff;"><b>LOCAL MATERIALS</b></td>
      </tr>
    ...
    <thead>
      ...
      <tr class="all_loc loc_area_body_<?='manpower'?>">
        ...
        <th <?=@$quotation->confirmed==1 ? 'colspan=3' : ''?> >Quantity</th>
        <th>Currency</th>
        <th>Unit Cost</th>
        <th colspan="6"></th> 
        <th>Total Price (Net)</th>
    ...
    <tbody>
      ...
      <tr id="q_items<?=$rs->id?>" class="loc_area_<?=$lrs->id?> loc_area_body_<?='manpower'?> all_loc supp_<?=$rs->supplier?> all_rows">
        ...
        <td <?=@$quotation->confirmed==1 ? 'colspan=3' : ''?> class="td_curreny"><?=number_format($rs->qty)?></td>
        <td>QR</td>
        <td class="td_curreny"><?=$rs->unit_cost?></td>
        <td colspan="6"></td>  
        <td class="td_curreny"><?=number_format($rs->unit_cost * $rs->qty, 2); $ttl+=($rs->unit_cost * $rs->qty);?></td>
        ...
        <td class="td_curreny"><?=number_format((($rs->unit_cost/(1-($rs->margin/100)))-$rs->unit_cost)*$rs->qty,2);?></td>
        <td class="td_curreny"><?=number_format($rs->unit_cost/(1-($rs->margin/100)),2); $r_unt=($rs->unit_cost/(1-($rs->margin/100)))?></td>
        <td class="td_curreny"><?=number_format($r_unt*$rs->qty,2); @$all_manpower_ttl+=$r_unt*$rs->qty;?></td> 
      </tr>
      <?php }}?> 
      <tr style="background-color: #e6ebf2;">
        <td colspan="<?=@$quotation->confirmed==1 ? '16' : '14'?>"><b>TOTAL</b></td>
    ...
    <tbody>
      ...
      <tr id="q_items_other" class="loc_area_other loc_area_body_<?='other'?> all_loc supp_other all_rows">
        ...
        <td> </td>
        <td><?=$rs->title?></td> 
        <td <?=@$quotation->confirmed==1 ? 'colspan=3' : ''?> class="td_curreny">1</td>
        <td>QR</td>
        <td class="td_curreny"><?=number_format(@$arr_items['OTHER'][$oid]->unit_cost,2)?></td>
        <td colspan="6"></td>
        <td class="td_curreny"><?=number_format(@$arr_items['OTHER'][$oid]->unit_cost,2); $ttl_other_amt+=@$arr_items['OTHER'][$oid]->unit_cost?></td>  
        <td> </td>
        <td class="td_curreny"> </td>
        <td class="td_curreny"> </td>
        <td class="td_curreny"> </td>
        <td class="td_curreny"><?=number_format(@$arr_items['OTHER'][$oid]->unit_cost,2);?></td>  
        <td class="td_curreny"><?=number_format(@$arr_items['OTHER'][$oid]->unit_cost,2);?></td>  
    ...
    <tbody> 
      <tr id="q_items_sla" class="loc_area_other loc_area_body_<?='sla'?> all_loc supp_sla all_rows">
        ...
        <td <?=@$quotation->confirmed==1 ? 'colspan=3' : ''?> class="td_curreny"><?=@$quotation->sla_amount ? 1 : 0?></td>
        <td>QR</td>
        <td class="td_curreny"><?=number_format(@$quotation->sla_amount,2)?></td>
        <td colspan="6"></td>
        <td class="td_curreny"><?=number_format(@$quotation->sla_amount,2); $ttl_sla_amount+=@@$quotation->sla_amount?></td>   
        <td></td>
        <td class="td_curreny"> </td>
        <td class="td_curreny"> </td>
        <td class="td_curreny"> </td>
...
<script type="text/javascript">
  ...
  $('#proj_amt').val('QAR <?=number_format($m_cost = @$all_lcnet+$manpower_amt_ttl+$local_amt_ttl+$ttl_other_amt,2)?>');

  $('#proj_cost').val('QAR <?=number_format($m_amt = @$all_net_mar_ttl+$manpower_cost_ttl+$local_cost_ttl+$ttl_other_amt+$ttl_sla_amount,2)?>');
